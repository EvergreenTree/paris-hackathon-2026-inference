...
def build_backend() -> InferenceBackend:
    ...
    if backend_name in {"custom", "tpfused"}:
        try:
            from server.custom_backend import TPFusedMoeBackend
            return TPFusedMoeBackend()
        except Exception as exc:
            LOG.warning("Falling back to HuggingFace Backend: %s", exc)
...
class InferenceEngine:
    ...
    async def _worker_loop(self) -> None:
        while True:
            ...
            # Opportunistically grab more requests, up to max_batch=64
            # We assume HACKATHON_MAX_BATCH_SIZE is capped
            max_batch = int(os.getenv("HACKATHON_MAX_BATCH_SIZE", "64"))
            num_retrieved = 1
            
            while num_retrieved < max_batch:
                try:
                    req_next, fut_next = self._queue.get_nowait()
                    batch_reqs.append(req_next)
                    batch_futs.append(fut_next)
                    num_retrieved += 1
                except asyncio.QueueEmpty:
                    break
            
            try:
                # If backend has `generate_batch`, use it, else fallback to loop
                if hasattr(self._backend, "generate_batch"):
                    responses = self._backend.generate_batch(batch_reqs)
                    for response, fut in zip(responses, batch_futs):
                        if not fut.done():
                            fut.set_result(response)
                else:
                    for req, fut in zip(batch_reqs, batch_futs):
